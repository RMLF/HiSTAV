...
This source file / compilation unit is part of HiSTAV.

HiSTAV is free software: you can redistribute it and/or modify
...
int main(){
	...
	std::cout << "HiSTAV Pre-Processor 2020(C) by Daniel Conde & Rui Ferreira @ CEris IST-ULisbon [v1.0]" << std::endl;
	std::cout << "******************************************************************************************************" << std::endl << std::endl;
	std::cout << std::endl;
	...
	if (fileExists(inputFileName)){
		...
		mesh.setInterpolatedData(frCoefRaster, "frCoef");
    ...
    inputFileName = mesh.control.rasterFolder + mesh.control.infiltCoefFileName;
    if (fileExists(inputFileName)){
        regularGrid permRaster;
        std::cout << "  Opening infiltration coefficient file: " << mesh.control.infiltCoefFileName << std::endl << std::endl;
        permRaster.importRaster(inputFileName, "infiltCoef");
        mesh.setInterpolatedData(permRaster, "infiltCoef");
        toLog("Friction coefficient raster parsed", 1);
        std::cout << std::endl;
        permRaster.clear();
        mesh.control.hasInfiltrationCoef = true;
    }else
        std::cout << "  No infiltration coefficient file" << std::endl;
