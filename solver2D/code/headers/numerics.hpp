...
public:
	...
	double flux[maxCONSERVED + maxSCALARS]; // separate
	double* dt;
};

// Hflux->dt  // acesso ao dt (apontador)
// Hflux.flux // acesso aos fluxos (local)

class elementFlow{ // ponto de acesso unico a toda a info
public:
    ...
    // 1 . CPU GPU void computeFluxes_Alternative();
	CPU GPU void applyFluxes_ReducedGudonov_1stOrder();

	CPU GPU void applyVelocityCorrections();
	CPU GPU void validate();

	elementState* state; // estado hidro (apontador)
	elementScalars* scalars; // estado escalar (apontador)
	elementFluxes* flux; // fluxos (apontador) e vizinhos (apontadores)
