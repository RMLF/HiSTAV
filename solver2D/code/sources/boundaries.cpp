...
#	endif
    ...
    float bndDepthFactor = 2.0f*lowDepth;

	if (conditionType == bndDISCHRIV){
        ...
        float avgSurface = minZb;
        float sumSurface = 0.0f;
        unsigned wetGhosts = 0;

        for (unsigned i = 0; i < numElemGhosts; ++i) {
            float bed = elemGhost[i].link->flow->state->z;
            float depth = elemGhost[i].link->flow->state->h;
            float surface = bed + depth;
            if (depth >= bndDepthFactor) {
                sumSurface += surface;
                wetGhosts += 1;
            }
        }

        while (wetGhosts < min(2u, numElemGhosts)) {
            avgSurface += bndDepthFactor;
            sumSurface = 0.0f;
            wetGhosts = 0;
            for (unsigned i = 0; i < numElemGhosts; ++i) {
                float bed = elemGhost[i].link->flow->state->z;
                float depth = avgSurface - bed;
                float surface = bed + depth;
                if (depth >= bndDepthFactor) {
                    sumSurface += surface;
                    wetGhosts += 1;
        ...
        avgSurface = sumSurface / float(wetGhosts);
		for (unsigned i = 0; i < numElemGhosts; ++i){
            float bed = elemGhost[i].link->flow->state->z;
            float depth = elemGhost[i].link->flow->state->h;
            float length = elemGhost[i].edgeLength;

            if (avgSurface - bed >= bndDepthFactor)
                depth = avgSurface - bed;
            else
                depth = 2.0f*bndTOLERANCE;

            elemGhost[i].state.h = depth;
