...
#	endif
	...
	control = &cpuControl;  // & - address of ,  control = 0x0000123123

	control->physics = &cpuPhysics;  // -> access member through pointer ,   . access member through object
...
#			pragma omp critical
            {
                if (threadPrivateDt < cpuNumerics.dt)
                    cpuNumerics.dt = threadPrivateDt; // control->numerics->dt = threadPrivateDt;
            }
#			pragma omp barrier
...
#			pragma omp single
            {
                ...
                /*float mass = 0.0f;
                for (unsigned i = 0; i < cpuMesh.numElems; ++i){
                    mass += cpuMesh.elemFlow[i].state->h * cpuMesh.elemFlow[i].area;
                }*/
...
void simulationSTAV2D::readControlFiles(){
	...
	std::cout << "   -> Reading " + controlFolder + timeControlFileName << std::endl;  // << write to

	timeControlFile >> initialTime; // >> read from
...
void simulationSTAV2D::readMeshFiles(){
	...
	cpuInitial.setInitialConditions(); // -> openDA
