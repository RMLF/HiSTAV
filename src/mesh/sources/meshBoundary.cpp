...
void meshGen::importBoundaryShapeFiles(){
	...
	pDataset = OGRSFDriverRegistrar::Open((shapesFolder + boundariesFileName).c_str(), FALSE);
	...
	pDataset = OGRSFDriverRegistrar::Open((shapesFolder + boundaryPointsFileName).c_str(), FALSE);
	pLayer = pDataset->GetLayer(0);
	pLayer->ResetReading();
	LayerType = pLayer->GetGeomType();
	pFDefn = pLayer->GetLayerDefn();
