...
void timeseries::readData(std::string filename){
	...
	while (!timeSeriesFile.eof()){
		timeSeriesFile >> inputFloat;
		timeSeriesFile >> inputFloat2;
		addData(inputFloat, inputFloat2);
		getline(timeSeriesFile, inputText);
	}
}
...
#	else
	...
	if (exit_status == 0)
		...
		std::cout << "  Simulation cannot proceed. Close console? ";
